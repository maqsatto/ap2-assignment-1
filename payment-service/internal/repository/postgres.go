...
func (r *PaymentRepo) GetByOrderID(orderID string) (*domain.Payment, error) {
	...
	err := r.db.QueryRow(
		...
		orderID,
	).Scan(&p.ID, &p.OrderID, &txID, &p.Amount, &p.Status, &p.CreatedAt)
	if err != nil {
		return nil, err
	}
	if txID.Valid {
...
func (r *PaymentRepo) List() ([]domain.Payment, error) {
	rows, err := r.db.Query(
		"SELECT id, order_id, transaction_id, amount, status, created_at FROM payments ORDER BY created_at DESC",
	)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		var txID sql.NullString
		if err := rows.Scan(&p.ID, &p.OrderID, &txID, &p.Amount, &p.Status, &p.CreatedAt); err != nil {
			return nil, err
		}
		if txID.Valid {
			p.TransactionID = txID.String
